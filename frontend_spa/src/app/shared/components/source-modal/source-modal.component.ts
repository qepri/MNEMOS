...
@Component({
  ...
  template: `
    @if (isOpen()) {
      <!-- Manual "modal" implementation since DaisyUI component styles are disabled -->
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity" (click)="close.emit()"></div>

        <!-- Modal Content -->
        <div class="relative w-full max-w-2xl bg-panel rounded-2xl shadow-2xl border border-divider overflow-hidden anime-fade-in flex flex-col max-h-[85vh]">
          <!-- Header -->
            ...
            @if (source()) {
              <div class="space-y-4">
                ...
                <div>
                  ...
                  <p class="mt-1 text-primary font-semibold flex items-center gap-2">
                    {{ source()!.document }}
                    @if (source()!.location) {
                      <span class="badge badge-sm badge-ghost font-normal">{{ source()!.location }}</span>
                    }
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div class="mt-2 p-4 bg-input rounded-lg border border-divider">
                    <p class="text-primary whitespace-pre-wrap leading-relaxed">{{ source()!.text }}</p>
                  </div>
                </div>
          ...
          <!-- Footer -->
          <div class="flex justify-end p-6 border-t border-divider bg-panel/50 mt-auto">
            <button
              (click)="close.emit()"
              class="btn-primary">
              Close
